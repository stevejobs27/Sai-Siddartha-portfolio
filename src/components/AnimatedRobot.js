...
export default function AnimatedRobot() {
  ...
  useEffect(() => {
    ...
    // FIX MUSIC NOTES: Clean previous notes first
    ...
    // FIX PAUSE BUTTON: Add event listener with inline style
    setTimeout(() => {
      const chargeElement = document.getElementById("charge");
      if (chargeElement) {
        // Set cursor directly as an inline style
        chargeElement.setAttribute("style", "cursor: pointer !important");
        
        // Add click event listener
        chargeElement.addEventListener("click", function(e) {
          // Prevent event from bubbling up to robot
          e.stopPropagation();
          ...
          if (isPaused) {
            ...
            // Change color to indicate paused state
            gsap.to("#charge circle", {
              fill: "#ff5252", // Red color to indicate paused
              duration: 0.3
            });
            
            // Make robot look back
            const lookBackTl = gsap.timeline()
              .to("#robot", {
                scaleX: -1,
                duration: 0.5,
                transformOrigin: "center",
        ...
        console.log("Pause button initialized with pointer cursor");
      } else {
        console.warn("Charge element not found");
      }
    ...
    return () => {
      ...
      if (robotEl) {
        robotEl.removeEventListener("click", () => {});
      }
      
      const chargeEl = document.getElementById("charge");
      if (chargeEl) {
        chargeEl.removeEventListener("click", () => {});
      }
  ...
  return (
    <div style={{ position: "relative", width: 800, height: 600 }}>
      ...
      {/* Robot SVG - All SVG content kept the same */}
      <div className="svg-box" style={{ width: 800, height: 600, position: "absolute", zIndex: 1 }}>
